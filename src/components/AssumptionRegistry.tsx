                ...
                if (editing === d.key) return (
                  <tr key={d.key} className="bg-sky-50/40"><td colSpan={9} className="px-3 py-2">
                    <form action={saveAssumption} className="flex flex-wrap items-end gap-2">
                      ...
                      <label className="block"><span className="block text-[9px] font-semibold uppercase text-slate-500">Low</span><input name="low" type="number" step="any" defaultValue={mine?.low ?? ""} placeholder={String(r.low)} aria-label={`Low estimate of ${d.label}`} className={inp + " w-24 text-right"} /></label>
                      <label className="block"><span className="block text-[9px] font-semibold uppercase text-slate-500">High</span><input name="high" type="number" step="any" defaultValue={mine?.high ?? ""} placeholder={String(r.high)} aria-label={`High estimate of ${d.label}`} className={inp + " w-24 text-right"} /></label>
