    ...
    <div className="space-y-4">
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-3">
        <div className="flex flex-wrap items-center gap-2">
          ...
          {preset === "custom" && <span className="flex items-center gap-1 text-xs"><input type="date" value={customFrom} aria-label="Window start" onChange={(e) => setCustomFrom(e.target.value)} className="rounded border border-slate-300 px-1.5 py-0.5" /> → <input type="date" value={customTo} aria-label="Window end" onChange={(e) => setCustomTo(e.target.value)} className="rounded border border-slate-300 px-1.5 py-0.5" /><button onClick={() => go("custom")} className="rounded bg-slate-800 px-2 py-0.5 text-[11px] font-medium text-white">go</button></span>}
