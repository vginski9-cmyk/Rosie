...
function SupplyChart({ need, rows }: { need: number; rows: { label: string; value: number; color: string; note: string }[] }) {
  ...
  return (
    <div className="space-y-2">
      ...
      <p className="text-[10px] leading-snug text-slate-500">Ceilings, not plans. The first bar is rooms on the dates the sections fall on, summed over the window; the second holds each site to its students-at-once and counts no seat beyond the day it is needed — the placed share on the left can never exceed it, and the gap between them is what could not be placed around (class days, holidays, sections too big for one site).</p>
    </div>
  );
}
...
export function SchedulerCapacity({ plan, policy, window, computing, mode = "diagnostic", onShowBottlenecks, onOpenLevers }: { plan: Plan; policy: Policy; window: { from: string; to: string }; computing?: boolean; /** diagnostic: a supply estimate, nothing is written; operational: the plan can be applied. */ mode?: "diagnostic" | "operational"; onShowBottlenecks: () => void; onOpenLevers?: () => void }) {
  ...
  const linedUp = Math.min(have, c.supplySeatsLinedUp);
      ...
      <div className={`rounded-2xl border border-slate-200 bg-white p-5 shadow-sm ${computing ? "opacity-60" : ""}`}>
        <div className="grid gap-6 lg:grid-cols-[232px_1fr_minmax(280px,38%)] lg:items-center">
          ...
          <SupplyChart need={need} rows={[
            { label: "Seats at the sites that count, on those dates", value: have, color: "#7fb0e8", note: "every open asset-shift on the dates and shift blocks the sections fall on, in every setting their rotation rules allow, at sites allowed by the Sites and Drive-time levers, × learners per shift — rooms, before any site's students-at-once" },
            { label: "…that line up with the demand, day by day", value: linedUp, color: RING.placed, note: "the same seats counted shift by shift, each site held to its approved students-at-once, and never more than that shift's own demand — a seat on a day nobody needs it does not count. Placed can never exceed this." },
            { label: "…of which a preceptor on the site's roster could cover", value: staffable, color: RING.staffed, note: `the seats at the sites that count, capped per site and shift by the preceptors on its roster × ${policy.studentsPerPreceptor ? `${policy.studentsPerPreceptor} students each` : "each asset's own students-per-preceptor ratio"}` },
        ...
        <details className="mt-3 text-xs text-slate-600">
          ...
          <dl className="mt-2 grid gap-x-6 gap-y-1.5 md:grid-cols-2">
            ...
            <div><dt className="font-semibold text-slate-800">Seats at the sites that count · {n0(have)}</dt><dd>Rooms: every seat that existed on the sections&apos; own dates and shift blocks, at sites the Sites and Drive-time levers allow, in every eligible setting, summed over the window. It says nothing about whether the seats fall where the students are — a Tuesday seat cannot host a Thursday section, forty students cannot use twenty-four seats on the same day, and a site&apos;s agreement may cap students below its rooms.</dd></div>
            <div><dt className="font-semibold text-slate-800">…that line up with the demand · {n0(linedUp)}</dt><dd>The same seats matched to the demand shift by shift{policy.flexibleDays || policy.flexibleShift ? " (week by week, and across shift blocks, where the Day and Shift levers let a shift move)" : ""}: each site held to its approved students-at-once, and never more than that shift needs. This is the ceiling the placed share is really measured against. Between it and placed{clashSeats || tooBigSeats ? ` — ${n0(Math.max(0, linedUp - s.placedSeats))} learner-shifts here —` : ""} sit only the clashes the engine cannot place around: class days, holidays, students due elsewhere{tooBigSeats ? ", sections too big for one site" : ""}.</dd></div>
