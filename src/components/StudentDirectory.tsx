...
import { SEX, RACE_ETHNICITY, RESIDENCY, PRIOR_EDUCATION, EMPLOYMENT_STATUS, NC_COUNTIES, AGE_BANDS, ageOn, ageBand } from "@/lib/learners";
...
export interface DirStudent {
  ...
  dob?: string | null; sex?: string | null; raceEthnicity?: string | null; county?: string | null; city?: string | null; residency?: string | null; priorEducation?: string | null; employmentStatus?: string | null;
}
...
export function StudentDirectory({ students, institutions }: { students: DirStudent[]; institutions: InstTree[] }) {
  ...
  const filtered = useMemo(() => {
    ...
    return students.filter((s) => {
      ...
      if (fAge && ageBand(ageOn(s.dob ?? null, today)) !== fAge) return false;
      if (fCounty && (s.county ?? "") !== fCounty) return false;
      if (fRes && (s.residency ?? "") !== fRes) return false;
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-end gap-3 rounded-xl border border-slate-200 bg-white p-4">
        ...
        <label className="block"><span className="mb-1 block text-[10px] font-semibold uppercase tracking-wide text-slate-400">Sex</span><select value={fSex} onChange={(e) => setFSex(e.target.value)} className="rounded-lg border border-slate-300 px-2.5 py-1.5 text-sm"><option value="">All</option>{SEX.map((x) => <option key={x} value={x}>{x}</option>)}</select></label>
        <label className="block"><span className="mb-1 block text-[10px] font-semibold uppercase tracking-wide text-slate-400">Race / ethnicity</span><select value={fRace} onChange={(e) => setFRace(e.target.value)} className="w-44 rounded-lg border border-slate-300 px-2.5 py-1.5 text-sm"><option value="">All</option>{RACE_ETHNICITY.map((x) => <option key={x} value={x}>{x}</option>)}</select></label>
        ...
        <table className="min-w-full border-collapse text-sm">
          <thead>
            <tr className="bg-slate-50 text-[11px] uppercase tracking-wide text-slate-500">
              ...
              <th className="px-3 py-2 text-left font-semibold">Demographics</th>
          ...
          <tbody className="divide-y divide-slate-100">
            {(showAll ? filtered : filtered.slice(0, LIMIT)).map((s) => (
              <tr key={s.id} className="hover:bg-slate-50/60">
                ...
                <td className="px-3 py-2 text-[11px] text-slate-500">{[s.sex, s.dob ? `${ageOn(s.dob, today)} y` : null, s.raceEthnicity, s.county ? `${s.county} Co.` : null, s.residency].filter(Boolean).join(" · ") || <span className="text-slate-300">not coded</span>}</td>
...
function EnrollForm({ institutions, onDone }: { institutions: InstTree[]; onDone: () => void }) {
  ...
  return (
    ...
    >
      ...
      <div className="lg:col-span-4 text-[10px] font-semibold uppercase tracking-wide text-slate-400">Demographics (coded — every one a dropdown or a date)</div>
      <label className="block"><span className="mb-1 block text-[10px] font-semibold uppercase tracking-wide text-slate-500">Date of birth</span><input name="dob" type="date" className="w-full rounded-lg border border-slate-300 px-2.5 py-1.5 text-sm" /></label>
      <label className="block"><span className="mb-1 block text-[10px] font-semibold uppercase tracking-wide text-slate-500">Sex</span><select name="sex" className="w-full rounded-lg border border-slate-300 px-2.5 py-1.5 text-sm"><option value="">—</option>{SEX.map((x) => <option key={x} value={x}>{x}</option>)}</select></label>
      <label className="block"><span className="mb-1 block text-[10px] font-semibold uppercase tracking-wide text-slate-500">Race / ethnicity</span><select name="raceEthnicity" className="w-full rounded-lg border border-slate-300 px-2.5 py-1.5 text-sm"><option value="">—</option>{RACE_ETHNICITY.map((x) => <option key={x} value={x}>{x}</option>)}</select></label>
      <label className="block"><span className="mb-1 block text-[10px] font-semibold uppercase tracking-wide text-slate-500">County</span><select name="county" className="w-full rounded-lg border border-slate-300 px-2.5 py-1.5 text-sm"><option value="">—</option>{NC_COUNTIES.map((x) => <option key={x} value={x}>{x}</option>)}</select></label>
      <label className="block"><span className="mb-1 block text-[10px] font-semibold uppercase tracking-wide text-slate-500">City</span><input name="city" className="w-full rounded-lg border border-slate-300 px-2.5 py-1.5 text-sm" /></label>
      <label className="block"><span className="mb-1 block text-[10px] font-semibold uppercase tracking-wide text-slate-500">ZIP</span><input name="zip" className="w-full rounded-lg border border-slate-300 px-2.5 py-1.5 text-sm" /></label>
      <label className="block"><span className="mb-1 block text-[10px] font-semibold uppercase tracking-wide text-slate-500">Residency</span><select name="residency" className="w-full rounded-lg border border-slate-300 px-2.5 py-1.5 text-sm"><option value="">—</option>{RESIDENCY.map((x) => <option key={x} value={x}>{x}</option>)}</select></label>
      <label className="block"><span className="mb-1 block text-[10px] font-semibold uppercase tracking-wide text-slate-500">Prior education</span><select name="priorEducation" className="w-full rounded-lg border border-slate-300 px-2.5 py-1.5 text-sm"><option value="">—</option>{PRIOR_EDUCATION.map((x) => <option key={x} value={x}>{x}</option>)}</select></label>
      <label className="block"><span className="mb-1 block text-[10px] font-semibold uppercase tracking-wide text-slate-500">Employment</span><select name="employmentStatus" className="w-full rounded-lg border border-slate-300 px-2.5 py-1.5 text-sm"><option value="">—</option>{EMPLOYMENT_STATUS.map((x) => <option key={x} value={x}>{x}</option>)}</select></label>
