          ...
          {changes.map((c) => (
            <li key={c.id} className={`px-3 py-2 ${c.undoneAt ? "text-slate-400" : "text-slate-700"}`}>
              <div className="flex flex-wrap items-baseline justify-between gap-2">
                ...
                {!c.undoneAt && onUndo && <button type="button" onClick={() => onUndo(c.id)} disabled={pending} className="rounded-lg border border-amber-300 bg-amber-50 px-2 py-0.5 font-medium text-amber-900 hover:bg-amber-100 disabled:opacity-50">Undo</button>}
              </div>
