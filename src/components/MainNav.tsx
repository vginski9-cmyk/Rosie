...
// scenarios that test expansion, the capacity that constrains it, and Setup (connections, mappings,
...
const ITEMS: { label: string; href: string; match: (p: string) => boolean }[] = [
  ...
  { label: "Programs", href: "/programs", match: (p) => (p.startsWith("/programs") && !p.endsWith("/expand")) || p.startsWith("/courses") || p.startsWith("/families/") || p.startsWith("/students") },
  { label: "Scenarios", href: "/scenarios", match: (p) => p.startsWith("/scenarios") || p.endsWith("/expand") },
