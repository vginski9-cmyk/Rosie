  ...
  const Row = ({ s }: { s: Setup["sites"][number] }) => {
    ...
    return (
      <tr className="hover:bg-slate-50/60">
        ...
        <td className="px-2 py-2 align-top whitespace-nowrap text-slate-600">{cap != null ? `${cap} at once` : <span className="font-medium text-amber-700" title="no students-at-once or approved capacity on record — unknown, not unlimited">cap unknown</span>}{fam.capacityBasis === "cases" && <span className="block text-[10px] text-slate-500">{s.casesPerDay != null ? `≈ ${dec(s.casesPerDay, 1)} cases/day${s.casesPerDaySource === "estimated from annual cases" ? " (estimate)" : ""}${s.annualSurgicalCases != null ? ` · ${fmt.num(s.annualSurgicalCases)}/yr` : ""}` : "no case volume — unknown"}</span>}{(s.daysAllowed || s.blocksAllowed) && <span className="block text-[10px] text-slate-400">{[s.daysAllowed, s.blocksAllowed].filter(Boolean).join(" · ")}</span>}</td>
        ...
        <td className="px-2 py-2 align-top whitespace-nowrap text-right">
          ...
          <span className={`block pt-1 text-[10px] ${s.setupDone === s.setupSteps ? "text-emerald-700" : "text-slate-600"}`}>{s.setupDone} of {s.setupSteps} steps</span>
        </td>
      </tr>
    );
  };
