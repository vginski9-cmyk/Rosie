...
export function AssetMapBoard({ institutionId, assets, overrides, bookings, rotations, cohorts, from, to, year }: {
  ...
  const rows: DatedInstance[] = useMemo(() => (!hydrated ? [] : cohorts.flatMap((c) => buildInstances({
    ...
    termStartByIndex: Object.fromEntries(Object.entries(c.termStartByIndex).map(([k, v]) => [k, v ? new Date(v) : null])), termEndByIndex: c.termEndByIndex, termWeeksByIndex: c.termWeeksByIndex, holidays: c.holidays, holidayRule: c.holidayRule, courses: c.courses,
