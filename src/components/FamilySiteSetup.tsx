    ...
    <div className="space-y-4">
      ...
      <Section id="availability" n={next()} title={`Availability agreed for ${fam.name}`} sub={fam.capacityBasis === "cases" ? `counted by cases: daily cases ÷ ${dec(fam.casesPerStudentDay ?? 2)} per student-day` : fam.capacityBasis === "staff" ? `counted by staff: qualified staff on shift × ${dec(fam.studentsPerStaff ?? 1)} students` : `counted by seats on the assets below, capped by students-at-once${fam.accreditor ? ` and the ${fam.accreditor} approval` : ""}`}>
        ...
        <form action={updateSiteAvailability.bind(null, fam.id, site.id)} className="grid gap-2 text-xs sm:grid-cols-2 lg:grid-cols-6">
          <label className="block"><span className={lbl}>Students at once</span><input name="studentsAtOnce" type="number" min="0" step="1" defaultValue={fs?.studentsAtOnce ?? ""} placeholder={fs?.approvedCapacity != null ? `${fs.approvedCapacity} (approved)` : "unknown"} className={inp + " w-full"} /><select name="studentsAtOnceMode" defaultValue={fs?.studentsAtOnceMode ?? "unknown"} className={inp + " mt-1 w-full"} aria-label="What a blank students-at-once means"><option value="unknown">blank = unknown (not asked)</option><option value="unrestricted">blank = explicitly no limit from this field</option></select></label>
          <label className="block lg:col-span-2"><span className={lbl}>Availability to {fam.name} students</span><select name="availabilityMode" defaultValue={fs?.availabilityMode ?? "inherit"} className={inp + " w-full"}><option value="inherit">use the assets&apos; schedules (whenever they run)</option><option value="specific">set specific days and shifts below</option><option value="unavailable">unavailable to this program</option><option value="unknown">unknown — not yet agreed</option></select><span className="block text-[10px] text-slate-500">{fs?.availabilityMode === "inherit" || !fs?.availabilityMode ? `resolved from ${liveAssets.length} asset schedule${liveAssets.length === 1 ? "" : "s"}; other limits (students at once, agreement dates) still apply` : fs.availabilityMode === "specific" ? "the days and shifts below" : fs.availabilityMode === "unavailable" ? "no shift here counts as open" : "nothing here counts as open or closed until answered"}</span></label>
      ...
      <Section id="assets" n={next()} title="Assets & shift structures" sub={`every ${d.settings.join(", ")} room, unit or machine ${fam.name} students can be placed on here`} right={<span className="text-xs text-slate-500">{familyAssets.length} asset{familyAssets.length === 1 ? "" : "s"} · {Object.entries(seatsBySetting).map(([k, v]) => `${k} ${v}`).join(" · ") || "none yet"}</span>}>
        ...
        <div className="mt-3 rounded-xl border border-slate-200 bg-white p-3"><SiteCapabilityPanel employerId={site.id} {...(() => { const r = capabilityRows(d.capabilityRaw); return { capabilities: r.capabilities, assets: r.assetLimits.filter((a) => familyAssets.some((f) => f.id === a.id)) }; })()} /></div>
