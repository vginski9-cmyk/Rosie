...
import { parseRuleSpec, ruleFromLegacy, KNOWN_SETTINGS, eligibleSettings, type SettingRuleSpec } from "./settingrule";
...
export async function requirementLedger(programId: string): Promise<{ requirements: LedgerRequirement[]; familyId: string | null }> {
  const program = await prisma.program.findUnique({ where: { id: programId }, select: { id: true, familyId: true, terms: { select: { courses: { select: { id: true, code: true, name: true, sessions: { select: { id: true, kind: true, number: true, title: true, lengthHours: true } } } } } } } });
  ...
  const rows = await prisma.clinicalRequirement.findMany({
  ...
  const requirements: LedgerRequirement[] = rows.map((r) => {
    ...
    const fulfillments = r.fulfillments.map((f) => ({ id: f.id, sessionId: f.sessionId, courseId: f.courseId, label: f.sessionId ? sessionLabel.get(f.sessionId) ?? "session" : f.courseId ? courseLabel.get(f.courseId) ?? "course" : "—", amount: f.amount }));
    const represented = fulfillments.reduce((n, f) => n + (f.amount ?? 0), 0);
    const version = pub ? {
    ...
    return { id: r.id, scope: r.scope, key: r.key, label: r.label, scopeLabel, version, draft: draft ? { id: draft.id, version: draft.version, updatedAt: draft.updatedAt.toISOString() } : null, versions: r.versions.length, fulfillments, represented, unresolved: version?.quantity != null ? Math.max(0, version.quantity - represented) : 0 };
  });
