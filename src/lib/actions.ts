...
import { writeSchedulerPlan, clearSchedulerPlanRows, AUTO_PLAN_NOTE as PW_AUTO_PLAN_NOTE, type AppliedPlan } from "./planwrite";
...
// ---------------------------------------------------------------------------
const AUTO_PLAN_NOTE = PW_AUTO_PLAN_NOTE;
export type { PlanAssignmentInput, AppliedPlan } from "./planwrite";

const PLAN_REVALIDATE = ["/scheduler", "/calendar", "/employers", "/students", "/people", "/insights/clinical-sites", "/insights/coverage", "/insights/site-load", "/insights/staffing-need"];

/** Write a recommended plan to the database so every calendar shows it — see lib/planwrite for what is written. */
export async function applySchedulerPlan(_institutionId: string, assignments: import("./planwrite").PlanAssignmentInput[]): Promise<import("./planwrite").AppliedPlan> {
  requireOperational();
  const r = await writeSchedulerPlan(assignments);
  for (const p of PLAN_REVALIDATE) revalidatePath(p);
  for (const c of new Set(assignments.map((a) => a.cohortId))) revalidatePath(`/programs/[id]/offerings/${c}`, "page");
  return r;
}
...
export async function clearSchedulerPlan(cohortIds: string[]): Promise<void> {
  ...
  await clearSchedulerPlanRows(cohortIds);
  for (const p of PLAN_REVALIDATE) revalidatePath(p);
}
