...
export const GLOSSARY: GlossarySection[] = [
  ...
  },
  {
    ...
    terms: [
      { key: "clinical-site", term: "Clinical site (partner)", definition: "An employer that hosts clinical shifts. Its agreement with the college is none, prospect, asked, secured or declined; a family may hold its own agreement with the site, which wins over the institution's. A college's partner record is its own relationship with the site: the agreement, contacts, the drive time from its campus and the assets it may use.", retired: ["employer (as the noun for a hosting site)"], notThe: "The site itself, which lives once in the site registry whichever colleges approach it.", where: "Setup › Clinical sites & partners, a site's page, the family's site setup" },
      { key: "site-registry", term: "Site registry", definition: "Every clinical site in the world the platform knows, once: its name, system, facility type, address and beds. Several colleges can court the same hospital; each keeps its own partner record linked to the one registry record, and sees the others doing it.", where: "Capacity › Site registry, the Setup page's \"add a site\" picker, the shared-site chip on a site's page" },
      ...
      { key: "seat-and-full", term: "Seat, shift and \"full\" (site load)", definition: "A seat is one learner place on one asset for one shift block on one date — an asset with learners per shift 2 opens two seats on each shift it runs. The roster is placed seat by seat by the scheduler (the seed places the demo roster the same way, under the roster levers), and the site-load page reads those seats back: each student-shift carries the asset, the shift block and the date it was booked on. \"Full\" is measured shift by shift: students on a shift ÷ seats open that shift in the programs' settings, summed over the shifts the site hosts — a Day shift against Day seats, never an average over the day — and the fullest single shift is shown under it. A student-shift the scheduler could not seat is shown as \"no seat yet\" at its section's pattern site and is never load on that site's seats. Because the scheduler never places over an asset's learners per shift or a site's students-at-once, a site can only read over 100% on a shift where someone booked by hand.", retired: ["seats per day", "students per day as the fullness measure"], where: "Clinical site load (the Full column, the seat-by-seat panel, the Seat / Shift / Asset dimensions), the scheduler's \"on the calendar now\" line" },
      { key: "roster-levers", term: "Roster levers", definition: "The levers the roster on the calendar was placed with: every secured site at any drive time, any shift block, ± 2 days inside the week, never on a holiday, seats first (a site's preceptor goes on the shift when it has one; none is required to place it). The scheduler opens on them, so its \"can be placed\" estimate and the site-load page start from the same rules; \"reset to the roster levers\" returns to them.", where: "The scheduler's levers card, the seed" },
  ...
  },
  {
    ...
    terms: [
      ...
      { key: "holiday-rule", term: "Holiday rule", definition: "How a class, lab or clinical session that lands on an observed holiday is handled, set per college: moved to the next open day in the same week (the default), to the previous open day, or only flagged for someone to move by hand. A day the same course already uses that week is never chosen. A whole week the college is closed for (a break) is not a term week: the weeks after it slide a week later and the term ends a week later, under every rule. The imported college calendar is the only authority for which days are holidays once it exists.", retired: [], notThe: "A hand-made move of one occurrence is filed under the session's pattern date and always wins over the rule.", where: "Setup → Basics, the offering design page, coverage, the scheduler" },
