...
export const GLOSSARY: GlossarySection[] = [
  ...
  },
  {
    ...
    terms: [
      ...
      { key: "holiday-rule", term: "Holiday rule", definition: "How a class, lab or clinical session that lands on an observed holiday is handled, set per college: moved to the next open day in the same week (the default), to the previous open day, or only flagged for someone to move by hand. A day the same course already uses that week is never chosen; a whole-week break stays flagged. The imported college calendar is the only authority for which days are holidays once it exists.", retired: [], notThe: "A hand-made move of one occurrence is filed under the session's pattern date and always wins over the rule.", where: "Setup → Basics, the offering design page, coverage, the scheduler" },
