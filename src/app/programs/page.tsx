...
// Programs — one card per job, with the program templates that deliver it and the
// three things you do with a program: design and sequence it, run offerings, set up
// its clinical network. No goals here; those live under Home → North Star goals.
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="max-w-3xl text-sm text-slate-500">Each job has one or more program templates (the timeless design: terms, courses, every class, lab and clinical session) and the offerings that run them for real cohorts. Design the template, run an offering, set up the clinical network — in that order.</p>
        </div>
          ...
          return (
            <section key={f.id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
              <div className="flex flex-wrap items-start justify-between gap-2">
                ...
                <div className="text-right text-xs text-slate-500"><div className="text-xl font-bold tabular-nums text-slate-900">{f.running}</div>offering{f.running === 1 ? "" : "s"} running{f.students ? ` · ${fmt.num(f.students)} students` : ""}</div>
              </div>
                ...
                {f.programs.map((p) => (
                  <div key={p.id} className="px-3 py-2">
                    <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-sm">
                      ...
                      <span className="ml-auto text-xs text-slate-500">{p.running} running</span>
                    </div>
                    <div className="mt-1.5 flex flex-wrap gap-1.5 text-[11px]">
                      ...
                      <Link href={`/programs/${p.id}`} className="rounded-full bg-slate-100 px-2 py-0.5 font-medium text-slate-700 hover:bg-rose-100 hover:text-rose-700">2 · Offerings, students &amp; calendar</Link>
                      <Link href={`/families/${f.id}/clinical`} className="rounded-full bg-slate-100 px-2 py-0.5 font-medium text-slate-700 hover:bg-rose-100 hover:text-rose-700">3 · Clinical sites &amp; requirements</Link>
                    </div>
                  </div>
                ))}
              ...
              {cl && (
                <div className="mt-3 flex flex-wrap gap-2 text-[11px]">
                  ...
                  {cl.score && <span className={`rounded-full px-2 py-0.5 font-medium ${cl.score.requiredCovered === cl.score.required ? "bg-emerald-100 text-emerald-700" : "bg-rose-100 text-rose-700"}`}>{cl.score.requiredCovered} of {cl.score.required} required experiences have a secured provider</span>}
