...
export default async function ProgramPage({ params }: { params: { id: string } }) {
  ...
  if (!program) notFound();
  const [offerings, provenance] = await Promise.all([getProgramOfferings(params.id), getCalendarProvenance(program.institutionId)]);
    ...
    <div className="space-y-6">
      ...
      <section className="rounded-2xl border border-slate-200 bg-white shadow-sm">
        ...
        <form action={createOffering.bind(null, program.id)} className="flex flex-wrap items-end gap-2 border-t border-slate-100 px-5 py-3">
          <label className="block"><span className="mb-0.5 block text-[10px] uppercase tracking-wide text-slate-400">New offering</span><input name="name" required placeholder="e.g. Class of 2031" className="input-sm w-56" /></label>
