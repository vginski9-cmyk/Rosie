  ...
  return (
    <div className="space-y-6">
      ...
      <section className="card card-pad space-y-3">
        ...
        {offerings.length === 0 ? (
          <p className="text-sm text-slate-400">No offerings yet — create one below, or lock one in from the goal planner.</p>
        ) : (
        ...
        {!OPERATIONAL && <p className="border-t border-slate-100 pt-3 text-xs text-slate-500">A running offering&apos;s rooms, staff and students are imported actuals. Planning a new run is yours: create it here or lock it in from the <Link href={`/programs/${program.id}/goal`} className="text-rose-700 hover:underline">goal planner</Link>; to test what it <em>would</em> do first, use a <Link href={`/programs/${program.id}/expand`} className="text-rose-700 hover:underline">scenario</Link>.</p>}
