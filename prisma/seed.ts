...
function genSessions(c: CourseSeed, weeks: number) {
  ...
  if (c.weeklyClinicalHours > 0) {
    // A tenth of a faculty member oversees every precepted clinical shift — the owner's rule for the Radiography clinicals (2026-09-24).
    const p = c.clinical ?? CLINICAL_PROFILE[c.code] ?? { mode: "Preceptor-led", maxStudents: 1, faculty: 0.1, preceptors: 1 };
