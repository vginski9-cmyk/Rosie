...
async function auditInstitution(inst: { id: string; name: string }) {
  ...
  // B9b′: the three lined-up ceilings share one basis and keep their order: 0 ≤ staffable ≤ lined up ≤ every site ≤ demand.
  { const c = plan.summary.capacity; if (c.supplySeatsLinedUpStaffable < 0) err(tag("B9 staffable"), `staffable ${c.supplySeatsLinedUpStaffable} < 0`); if (c.supplySeatsLinedUpStaffable > c.supplySeatsLinedUp + 1e-9) err(tag("B9 staffable vs lined-up"), `staffable ${c.supplySeatsLinedUpStaffable} > lined up ${c.supplySeatsLinedUp}`); if (c.supplySeatsLinedUpEverySite < c.supplySeatsLinedUp) err(tag("B9 every-site vs lined-up"), `every site ${c.supplySeatsLinedUpEverySite} < lined up ${c.supplySeatsLinedUp}`); if (c.supplySeatsLinedUpEverySite > demandSeats) err(tag("B9 every-site vs demand"), `every site ${c.supplySeatsLinedUpEverySite} > demand ${demandSeats}`); }
