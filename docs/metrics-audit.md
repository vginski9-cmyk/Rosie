...
Lenoir's real meeting days never reached its sessions (the seed sets Mon/Wed templates and Mon/Sat bookings); 217 sessions land on coded holidays and are listed as exceptions; until 2026-09-22 completed cohorts carried no students and dummy names repeated across offerings (both fixed: every graduated class carries its full history, and a dummy name is never reused); every employer is geocoded to its town centroid. These are shown as what they are on the pages that read them.
...
**Class sizes at Sandhills (2026-09-21).** The owner's figures: Surgical Technology's maximum cohort is 20 and Radiography's is 23 (`maxCohort` in `prisma/templates/surgtech.json` and `rad.json`; the workbook's modeled enrollments of 19 and 41 stay as `modeledEnrollment`). A Radiography goal of 30 a year at a 90% completion rate needs about 41 seats, which one class of 23 cannot carry, so the seed now runs two Fall classes a year for the Classes of 2028, 2029 and 2030 — 15 workers and about 21 seats each — named "Class of 2028" and "Class of 2028 · 2nd class" (a second class graduating the same year says so, rather than "(2)"). The naming rule goes by length alone: a program of twenty weeks or less (or on demand) is named by its start and place; a two-year AAS that launches every Fall and Spring is still a class year. A January Radiography start is not modeled, because the template pins its terms to Fall, Spring, Summer, Fall, Spring.

### Instructors & preceptors needed, for classes that run past the semester (2026-09-21)

Every week of a session was bucketed under the season its *term* started in, so a Lenoir class that starts in November put its February weeks in "Fall", the semester windows read "Fall 1/6 → 12/30", and the "staffing plan by term" lumped thirty-three one-term classes into a single Term 1 row spanning two years. A week now belongs to the calendar semester its date falls in, by the college's own semester anchors (`semesterAt`; the capacity model hands each cohort its anchors), in the semester bars, the weekly bands and the clinical-by-rotation bands alike — for a Radiography term the buckets are unchanged, since a Fall term's weeks are Fall weeks. The by-term table is a by-semester table: the peak weekly need inside each semester, its real window, and the offerings in session then (with their term when a program has several). A continuing-education program's chosen dates no longer count as "set by hand" in the provenance banner: they are the class's own dates, not stand-ins for a calendar the college could have supplied. Offering names are unique across a family, not only within a program (Carteret runs one program per delivery model): "Aug 2026 · Morehead City" and "Aug 2026 · Morehead City · 11-week daytime"; Lenoir's same-month runs at one place carry their days and time of day.

### Clinical sites in Setup, and drive-time bands instead of rings (2026-09-21)

Clinical sites, their partners, assets and shifts were only reachable through a program's clinical pages or the Capacity › All organizations directory, and the Setup page mentioned them only as a fold inside Mappings. Setup now has a top-level section, "3 · Clinical sites & partners": six tiles (sites and how many are located, secured with asked and prospects, assets and how many are still estimates, seats a shift, preceptors on the sites' rosters, programs served), one row per site sorted by drive time from the main campus (type, drive minutes, umbrella agreement, the programs it serves as links to that program's site setup, assets, seats a shift, preceptors, a link to its assets & shifts), links to the by-program sites page, the scheduler and the map, and an "Add a clinical site or partner" form that creates the site, locates it from its address and computes its drive time. The create action now keeps the organization, facility type and umbrella agreement the form sends (it silently dropped them before). The readiness steps count "Clinical sites" as its own step (a secured agreement and at least one asset), and Mappings no longer depends on sites. Long lists scroll inside the table with the header pinned.

The words "Core", "Ring 1", "Ring 2" and "Ring 3" no longer appear anywhere a person reads: every badge, filter, lever, column heading, fix suggestion and site-load dimension says the drive-time band in minutes (≤ 30 min · 30–60 min · 60–90 min · over 90 min, from the college's own band settings where the page knows the college, else the defaults), and the scheduler's lever reads "within 30 min drive" and so on. The stored ring codes are unchanged (additive rule: `Employer.ring` keeps its values; `driveBandLabel` in `src/lib/geo.ts` is the one place they become words), so nothing that filters or sorts on them moved. The Basics form calls them "drive-time bands" with "nearest / second / third band, up to (min)". Missing ≠ zero: a site with no usable address still shows "not located", never a band.

### The shared site registry, and offering dates that agree with themselves (2026-09-21)

**Sites are no longer owned by a college.** A clinical site existed only as one college's partner record (`Employer`, with its `institutionId`), so two colleges approaching the same hospital would have kept two records with two addresses and never seen each other. There is now one `ClinicalSite` per site in the world (name, system, facility type, address, county, coordinates, beds, ORs, case volume) and every partner record points at it (`Employer.siteId`, additive, nullable). The partner record keeps what is genuinely the college's own: its agreement (none / prospect / asked / secured / declined), its contacts, the drive time and band from *its* main campus, and the assets, shifts and people it has been granted. The seed links every partner record to a registry record at the end (`linkSiteRegistry`, matched by normalised name and city; 180 partner records → 180 sites today, none shared yet), and a college adding a site through Setup finds or creates the registry record first. Correcting a site's identity on one college's record updates the registry record, so every college sees the correction.

- **Capacity › Site registry** (`/sites`): every site once, with a chip per college courting it (agreement, drive from that college's campus, assets and seats on that college's record) linking to that college's partner record; a filter by college ("its partners" / "not yet its partner"); an "add as a partner of…" form per row that creates the college's own partner record from the site's identity and times it from that college's campus; and a form for a site nobody has recorded.
- **Setup › Clinical sites & partners** offers "a site another college already knows" (a picker of registry sites the college has no record for, each saying which colleges already partner with it) above the blank form.
- A site's page carries a "shared site record" chip naming the other colleges that partner with it, each linking to their record.

Assets stay on the partner record on purpose: what a hospital lets one college use is that college's agreement, not a property of the building. Missing ≠ zero: a registry site nobody has courted shows "no college yet", not zero seats.

**Goal planner dates.** The planner's year is the year an offering *graduates* (its offerings are listed under the year they finish), but a suggested start was worked back from "fully productive by December 31" through twelve months of licensure, placement and ramp lags and then spaced backwards through a year — so a 2029 CNA goal produced classes starting in October 2027. And the end shown beside a start was a rough count of weeks, while the offering page aligned the real terms to the calendar (and the locked line on the planner used a third figure, start plus weeks). Now:

- A suggested start is one whose last term, aligned to the college's calendar exactly as the offering page aligns it (`alignOffering`), ends inside the goal year (`src/lib/goaldates.ts`). For a continuing-education class (calendar mode continuous) every open Monday from the year before through the goal year qualifies when its aligned end lands in the year; for a semester program, the coded semester starts (else the college's pattern anchors) whose aligned end lands in the year, and of term 1's coded season only. Several offerings are spaced evenly over those candidates, first and last used, so eight classes for 2029 run through 2029.
- Each unlocked offering's start is a picker of those candidates (any other date can still be typed); the end beside it is the aligned last day, and a start that would graduate in another year says so in amber. "Space the starts across {year}" re-spaces the unlocked offerings; adding an offering re-spaces them too. A year no start can still reach says so instead of suggesting anything.
- The offering's end is one date everywhere: the last term's real end as stored by alignment. `computeCohortTiming` takes real term ends and uses them (it had used start + weeks, the Monday after); the goal page passes them, and the offering page's header, tiles and term summary read that end, with "last class meets …" as a note when the final meeting falls earlier in the week. The "productive by" work-back (Phase 6) still drives the feasibility banner; it no longer picks start dates.

### The scheduler never places a student twice at once (2026-09-21)

The scheduler placed each clinical section independently and only afterwards *reported* "students placed in two places at once" as a blocker. With the day and shift levers loosened, a shift moved a day or a block could land on a date and block the same students already had a shift for, and the plan carried the double booking through to apply. That is now impossible rather than flagged:

- While placing, the plan keeps the seat numbers of every cohort it has put on each date × block. A date × block where any of a unit's seats are already placed is not a candidate at all, whatever it scores — a hard rule, not a lever. A moved shift is also kept off any date × block where the same students have a shift of their own on the calendar (placed yet or not), so a moved shift never takes the slot another of their shifts is meant to fill.
- A second shift that has nowhere else to go is left **unplaced** with the reason "these students are already on another clinical shift then — students can't be in two places", the specific sentence naming the students and the date, and the fixes: ± days or another block where that would place it, else "two clinical sessions put the same students on the same shift — move one to another day or shift block (design & sequence — this offering)".
- The readiness blocker stays as a safety net, now computed on seat ranges (two placements of the same cohort at one date × block whose seats overlap), so a split section's pieces never trip it and two sections sharing a date with different students never do either.
- Classes and labs were already hard: a clinical never moves onto a day the cohort is on campus and never overlaps a class's hours on its own day (`class-day`). Unchanged.

Checked on the seeded Sandhills data for September 2026 – August 2027: with exact dates and shifts, no overlap and nothing left unplaced for being busy; with any shift and ± 2 days, every shift placed, no overlap, nothing busy. The test fixtures that gave every section the same seat numbers (so "section 2" looked like section 1's students) now deal seats the way the real demand does.

### "Enough seats" beside unplaced shifts, and holiday shifts under the Day lever (2026-09-21)

The scheduler's headline said "You have enough seats and enough preceptors for every shift — 30,557 seats to spare" while the tiles counted shifts with no seat, and the tile called every unplaced shift a shift "with no seat" even when it was on a holiday or had no site under an allowed agreement. The window-wide headroom (seats across five years) was being read as if it answered the day-by-day question. Now:

- The verdict is built from what the plan actually left unplaced, by reason: "163 of 21,460 learner-shifts could not be placed under these levers: 127 every open asset is already full that shift; 18 the only sites with this setting aren't under an allowed agreement; 18 no asset of this setting runs that shift on that date. Seats overall are not the problem (… to spare across the window)." It says "every shift found a seat" only when that is true. The tile reads "Shifts left unplaced" with the same breakdown, and says in words that seats across the window are not the same as a seat on the day.
- A shift on an observed holiday moves off it inside its week when the Day lever allows ± days (never onto another holiday), and goes last in the placement order so it takes leftover seats rather than a regular shift's own. When no other day that week has a seat, its reason stays "lands on an observed holiday — needs moving", with the detail saying that no other day under the Day lever had a seat; the fixes offer the ± days that would place it. No shift of any kind is ever moved onto a holiday any more — the old plan had been moving some onto Labor Day and Easter Monday, which is why holiday weeks looked less full than they are.
- The hundreds of "already full" rows the reader saw came from the build before double booking was stopped: the same students placed twice were consuming the seats, so the last sections of each class found every site full. With double booking gone and no moves onto holidays, the seeded Sandhills window (Aug 2025 – Jun 2030, secured sites within 60 min, any shift, ± 2 days) places 21,297 of 21,460 learner-shifts; the 163 left are the holiday-week squeeze (two cohorts' shifts pushed into fewer days at the sites within 60 min), Surgical Technology's ambulatory setting with no secured site, and the Fall Break week where no other day has a seat.

### The scheduler page rebuilt around one number, and three rules made hard (2026-09-21)

**Rules.** A site's students-at-once (the family's agreement with it) is now a hard limit while placing — a site is never placed over it; the extra student is left unplaced with the reason "full". It had been a blocker reported after the fact. (On the seeded data the limit never binds: every partner's assets hold fewer learners a shift than its agreement allows, so placement is unchanged; the rule is there for the day it does.) A clinical with no stated start time is a Day, Evening or Night shift, not an all-day one: the class-overlap check had been treating it as clashing with any class that day. Every partner site's required experiences are recorded as confirmed by the seed (`prisma/seed-confirm.ts`, 2,950 provisions), so the readiness rung "experience confirmed by the site" holds and nothing reads "inferred only"; a real college records these one by one on the program's site page.

**The page.** The levers sit on top, always in view: the six that decide what can be placed (sites that count, drive time, shift, day, preceptors, holidays), the window and the offerings; the ten preferences fold under "all levers". The answer is one number — the share of clinical shifts placed — with the three rings beside it (found a seat → preceptor named → ready to run) and one chip for seats across the window as a multiple of need, expanded on demand into the three bars. Under it, "Why not 100%": each reason the plan left shifts unplaced with its count and the fix that recovers the most, each linking to the week-by-week bottlenecks; the blockers that would stop an apply sit in the same list with their fix. The preamble (what the page shows, its population, window, constraints, the sibling views and why they differ, the "dates set by hand" banner, the engine's statement, the read-only paragraph) is gone; the header says the college and, in the strategic product, "diagnostic — nothing is written to the calendar". Tabs: Bottlenecks & fixes (first), Week by week, Sites, Every shift, By student, By preceptor.

**Levers, swept.** Every combination of the hard-constraint levers (3 agreement tiers × 4 drive-time bands × 3 day settings × 2 shift settings, plus preceptor-required, holidays-anyway, spread, no continuity, never split, a 45-minute drive cap and 1:1 preceptors) was run on the seeded Sandhills year September 2026 – August 2027 and checked: nothing placed on an observed holiday, nothing over a site's students-at-once, no student in two places at once, and loosening any lever never placing fewer learner-shifts than the tighter setting. The first pass found two combinations (secured + asked sites, nearest band, ± 1 or 2 days) placing a site a few shifts over its limit: the after-the-fact check counted every family's learners at the site while the limit is one family's agreement. Both now count per family; the rerun is clean on every invariant.

Sweep results (Sandhills, Sept 2026 – Aug 2027, 5,595 learner-shifts; placed / left unplaced, with the reasons):

| Levers | Placed | Unplaced | Why |
|---|---|---|---|
| default: secured sites, any drive time, exact shift, exact date | 4,156 | 1,439 | 1,129 full that shift · 292 overlap a class · 18 no allowed agreement |
| secured, within 30 min, exact shift, exact date | 2,605 | 2,990 | 2,680 full · 292 class · 18 agreement |
| secured, within 60 min, any shift, ± 2 days | 5,523 | 72 | 36 full · 18 agreement · 18 no asset runs that shift |
| secured, any drive time, any shift, ± 2 days | 5,531 | 64 | 28 full · 18 agreement · 18 closed that day |
| secured + asked, any drive time, any shift, ± 2 days | 5,571 | 24 | 6 full · 18 closed that day |
| any partner, any drive time, any shift, ± 2 days | 5,577 | 18 | 18 closed that day (Surgical Technology's ambulatory setting) |
| any partner, loosest, preceptor required | 5,571 | 24 | 6 no free preceptor · 18 closed that day |
| default with a 45-minute drive cap from home | 3,937 | 1,658 | + 152 beyond the students' drive cap |
| default, one preceptor per student required | 3,970 | 1,625 | 543 no free preceptor at any open site |

Across all 80 plans: nothing on a holiday, nothing over a site's limit, no student in two places, and no looser setting ever placing fewer than a tighter one. The 292 class overlaps under exact dates are the seed's holiday rule moving Labor Day and Fall Break clinicals onto days the cohort has class — design conflicts the page now names, with the fix.

**Follow-up (2026-09-21).** The headline now reads "x% of clinical shifts can be placed" — an estimate of clinical supply under the levers, nothing placed — with demand against supply drawn as bars in the header (demand as the yardstick; supply at the sites that count, the part a preceptor could cover, and supply if every site counted, each as a multiple of demand). The rings are large enough to carry their number. In the strategic product the blockers read "not ready to run" rather than "would stop an apply". The week of 11 October 2027 on the seeded data: Wednesday the 13th is Fall Break; 38 one-student RAD-251 shifts of the two Class of 2028 cohorts fall on it; under ± 2 days and any shift, 27 move to Friday evening and night seats, and 10–11 have no seat on any other day of that week because Monday, Tuesday, Thursday and Friday day shifts are already full with those cohorts' own shifts and classes. The design's holiday rule could not move the session at design time for the same reason. The page names it: move that week's session on Design & sequence.

### One set of placements: the roster is the scheduler's plan, and site load reads it seat by seat (2026-09-21)

**What was wrong.** The site-load page read sixteen sites "Full" at 118–202% (Pinehurst Surgical Clinic 118.6%, FirstHealth Raeford 178%) while the scheduler found room for nearly everything. Two causes. The seeded roster was not placed by the scheduler at all: the seed dealt each clinical section to a site by round-robin (`hostSlots`) and pinned nothing to an asset, so several sections could sit at one site on the same day regardless of its seats. And "Full" was the wrong ratio: the average number of students on a *day* divided by the site's *Day-shift* seats, so a site running day, evening and night shifts read two or three times fuller than any shift ever was.

**One path for placements (`src/lib/planwrite.ts`).** The body of the apply action moved into a pure Prisma module with no request context, so the seed and the server action write a plan through the same code: a learner booking per section × asset × shift, each section's meeting pattern pointed at its main site, a calendar move for every shift that landed on another day or block, the site's preceptors on the shift, every student's shift pinned to the booked asset. `applySchedulerPlan` and `clearSchedulerPlan` keep the operational gate and the cache revalidation and delegate the writes.

**The seed places the roster with the scheduler (`prisma/seed-plan.ts`).** After the students exist and every site has confirmed its experiences, each college's demand is built exactly as the scheduler page builds it (`getCapacityModel` → `schedulerWindow` → `getSchedulerData` → `buildSchedulerPlan`) under the roster levers — `ROSTER_POLICY` in `src/lib/schedulerplan.ts`: every secured site at any drive time, any shift, ± 2 days inside the week, never a holiday, seats first — and written with `writeSchedulerPlan`. The scheduler page now opens on those same levers ("reset to the roster levers"), and shows what is on the calendar: the share of the roster's student-shifts sitting on a booked seat, the bookings and sites, and the date written. Seeded preceptors exist at every secured site whose assets serve a program's settings, not only at the pattern hosts, so a shift placed anywhere secured has someone of the discipline to precept it; the seed no longer hand-assigns clinical preceptor rows (the plan writes them).

Sandhills, first run against the previous roster (planned and running offerings, 2025-08-18 → 2030-06-03):

| | |
|---|---|
| Learner-shifts in demand | 21,460 |
| Placed | 21,327 (16 sites) |
| Left unplaced | 133 — 59 every open asset full that shift · 38 on a holiday with nowhere in the week to go · 18 no allowed agreement · 18 the only setting's site closed that day |
| Bookings written | 21,327 · asset-shifts over their learners per shift: **0** |
| Student-shifts pinned to a seat | 20,001 of 22,356 (the rest: completed offerings, withdrawn students, and the 133 unplaced seats) |

**Site load, seat by seat (`src/lib/siteload.ts`, `getSiteLoad`).** Each row now carries the seat: the asset (unit, room or suite), its shift block, its learners per shift, and the date the booking landed on (the plan may have moved it). "Full" is Σ students ÷ Σ seats open, shift by shift, over the shifts the site hosts — seats open on a shift are the site's active assets running that block in the settings of the families present, plus any asset actually in use — with the fullest single shift under it and, per site, a seat-by-seat table of every asset the roster booked (seats per shift, student-shifts, shifts used, fullest, fill). New dimensions in the query bar and pivot: asset, shift, seat (booked seat / no seat yet). A student-shift the plan could not seat is shown at its pattern site and counted as "no seat yet", never as load; the headline says how many and points at the scheduler. Because the engine never places over an asset's learners per shift or a site's students-at-once, a site can only read over 100% on a shift where a hand-made booking put it there — and "Watch" now flags the fullest shift, not an average. "Student-days" became "student-shifts" throughout (a student can have two shifts in a day).

**Drive-time bands everywhere.** The site-load "drive time from campus" dimension and CSV, and the supply map's by-region table, read "≤ 30 min / 30–60 min / 60–90 min", never Core / Ring 1 / Ring 2; the ring names survive only as the stored value behind `driveBandLabel`.

**Reconciliation strip.** The site-load view of the three-way bridge now shows its seated count ("21,110 student-shifts · 20,001 on a booked seat"), and the "why other views differ" line says the real difference: the scheduler counts every seat of every section at enrollment targets; site load counts the named students in them, so an unfilled or withdrawn seat is the gap.

**Two more gaps the seat ledger exposed.** With the roster read seat by seat, 955 future student-shifts at Sandhills had no seat although only 130 learner-shifts were unplaced. Most were the 20th and 21st students of offerings whose later terms planned 19 seats: a term's demand was its enrollment target thinned by the family's completion rate, so a named student still on the roster had no seat in the plan from term four on. Now a term never plans fewer seats than the students on the roster who have not withdrawn (`getCapacityModel`) — the scheduler, the capacity view and site load count the same people, and attrition shows up as withdrawals, not as seats planned away under students who are still there. The rest were shifts the seed had created for clinical sessions that never land on a date (an orientation coded on the Monday before a Tuesday term opens); the seed no longer makes a shift for a session with no date.

Tests: `test/siteload.test.ts` (10) covers the shift-by-shift ratio, evening seats against evening shifts, an asset in use always counting as open, no-seat rows never being load, an over-booked shift flagged by the fullest shift, and the band / asset / shift dimensions with no ring names in the CSV.

### Structured requirements, setting rules, sites as resources, supervision, AI-assisted inputs and one evaluation (2026-09-22)

**What was wrong.** A clinical rotation type was one setting code, so "Acute MedSurg or LTC" silently meant the hospital only: the SNF's seats never counted, the scheduler reported a hospital-only bottleneck, and nothing recorded that the wording had never been interpreted by a person. Supervision was a free-text mode plus two floats, so a blank count read as zero staff and every unfilled role was called "unprecepted" even for instructor-led groups. A blank students-at-once read as no limit. Requirements lived in course hours and credentialing sets with no version, no source and no link to the sessions that satisfy them. Each view read the rules its own way.

**What changed (phase 14; the full record is `docs/requirements-map.md`).** Setting rules are structured (`only` · `any-of` · `all-of` · `pool` with minimums · `n-of`, with mixing, continuity and scope) and carry the program's wording and an interpretation status; a compound wording is proposed, never approved. Requirements are versioned (draft → published → retired) with quantity, unit, basis, source authority and fulfillment links, and the ledger shows required · represented · unresolved. Sites gain capabilities (supported · limited · unsupported · unknown), explicit limit modes (known · unrestricted · unknown) and availability modes (inherit · specific · unavailable · unknown). Supervision is a role model; staff demand is groups × hours; a required role with no count is "policy missing", never zero. "Describe or upload" turns a workbook, a text PDF, a Word file or plain words into a validated proposal (deterministic parser always; Anthropic provider when configured) that a person accepts into drafts. One evaluation service judges every placement on eight checks with reason codes and is read by the scheduler, the exception queue, the staffing panels and the exports.

**Numbers that moved, and why.** Carteret, roster levers: placed 46.5% → 69.8% (the OR rule reaches LTC seats — a corrected interpretation), ready share unchanged at 0% (the rule is unreviewed, so the twenty extra placements are conditional; and no instructor is on record). Sandhills: placed unchanged at 99.6%; ready share 98.7% → 87.8% because 2,432 sections sit under compound wordings nobody has reviewed — a newly exposed unknown, not a regression. Lenoir and Roanoke-Chowan unchanged. A seated shift is no longer flagged "setting rule unmet" because a sibling section of the same rotation is unplaced (that was inflating the blocker to 3,226 shifts at Sandhills).

**Missing ≠ zero, again.** 106 secured site agreements have no students-at-once: every placement there is now an evidence gap (CAPACITY_UNKNOWN), not a pass. 252 clinical sessions have a supervision model that needs a decision. 16 rotation wordings need a reviewer's name. These are listed on Home as warnings and notes with their reason codes.

Tests: `test/settingrule.test.ts` (13), `test/supervision.test.ts` (8), `test/evaluate.test.ts` (6), `test/schedulerrules.test.ts` (11), `test/extraction.test.ts` (9), plus additions to `exceptions`, `workload`, `scheduler`, `validation` and `assetmap` — 546 tests, all passing. Not exercised: a live provider request (no key in this environment), OCR, and the server-bypass / concurrency paths beyond code review.

### The template is the source of truth: requirements as course and session fields, coverage derived (2026-09-22)

**What was wrong.** The requirements ledger sat beside the template with a hand-kept list of linked sessions per requirement (thirty chips for a 200-hour course) and a "link a session" picker. The course and session rows already say everything needed — the sessions' rotation types and the rules they mean — so the links were a second copy of the template that could drift from it.

**What changed.** A course's clinical requirements are fields inside its block on Design & sequence; what represents them is derived from the course's sessions through their setting rules (`src/lib/requirementcoverage.ts`), with a session that reaches several requirements held as "share to state" rather than split by guesswork. Every clinical session row shows and edits the setting rule its rotation type means (the AND / OR tags), lists the eligible settings from the college's taxonomy and the sites that carry them, and gains two template columns — experiences (what) and progression stage (when) — that import, export and extract like every other column. A rotation wording nobody mapped is tagged automatically from the taxonomy as a proposed rule, so demand reaches every eligible setting conditionally instead of reading unmapped.

**Numbers.** Unchanged for reviewed rotations; a previously unmapped wording now places conditionally under `requirement-unreviewed` instead of failing with `unmapped-setting`. Tests: `test/requirementcoverage.test.ts` (4) — 550 tests passing.

**Course rotation pools, and what they exposed.** Radiography's sessions are all tagged "General Rotations" / "Other (imaging rotations)" (GEN) while each course's requirements spread hours across GEN, ED, PORT, OR, FLUORO and CT. Every such course now reads a pool rule — every requirement setting, quantities as minimums — so the scheduler reaches all six settings and seats the minimums first, and coverage allocates the course's hours to the requirements in order. Reading the seed this way shows that RAD-161's minimums (240 h in a 240 h course) and RAD-251's (329 h in 336 h) need more whole eight-hour sessions than the courses have (34 of 32; 43 of 42): Sandhills' ready share under the rules drops from 87.8% to 34.9% on SETTING_MINIMUM_UNMET, a conflict in the seeded area hours the legacy calculation hid by placing everything in GEN. The course block states the count on each requirement so a person can adjust the area hours or the session length.

### Accuracy audit of the clinical pipeline (2026-09-22)

`scripts/audit-accuracy.ts` recomputes every clinical total from the raw rows and cross-checks template → demand → plan → roster → capacity view → site load → caps → staffing → requirements. Six defects, all fixed (docs/requirements-map.md §9): a demand unit's identity lacked the offering, so two offerings of one program merged in every count (Sandhills read 13,125 placed shifts for 21,733 placements); per-occurrence moves were not scoped to their offering; an applied plan's own moves seeded the next plan (± 2 days of drift per re-apply); re-applying rewrote the past and left logged shifts pinned to bookings that no longer existed, which site load counted as seats (234 asset-shifts over 100% that did not exist); and the capacity view matched demand to its primary setting only, reading 225 learner-shifts short that the scheduler placed in eligible alternatives. After the fixes the audit reports 0 errors for all four colleges; the remaining warnings are seed data (sessions a day past their stated term end).

**A seventh, from a reader's question.** The scheduler said 87.5% of shifts could be placed while "supply at the sites that count" read 0.45× demand. Placement used every setting a rotation's rule allows; the supply summary counted seats in each section's primary setting only, so a Radiography pool over six settings was measured against GEN seats alone. Supply on demand days now counts every eligible setting (that scenario: 16,493 seats, 1.42× demand, against 10,177 placed), and the per-setting balance and week rows attribute each section to the setting it landed in, so the rows add up and no setting reads over 100%. The audit now checks that supply on demand days is never below what was placed.

**Eighth and ninth, from the next question.** Under secured · within 30 min · exact shift · exact date, four Radiography offerings read 55.8% placed (6,496 of 11,640) beside 8,775 seats at the sites that count (0.75×). Tracing one day: 40 students, 24 eligible seats, 11 placed. The rotation's pool rule puts its GEN minimum first, and the engine restricted a section to the minimum's setting whenever that setting had an asset at all — GEN's three rooms filled and the 13 ED / PORT / OR / FLUORO seats beside them stayed empty while 29 students read "full". The minimum's setting now goes first only while a pool of it can seat the whole section; otherwise every eligible setting competes, with a scoring bonus keeping the minimum preferred (6,912 placed; the roster plan 21,739 → 21,776 of 21,812). The days that still fall short are Moore Regional's approved 16 students at once, which its 20+ eligible rooms do not show; that is now its own unmet reason (`site-cap`) with its own fix, not "every open asset is already full". And the panel gained the ceiling the placed share is really measured against — seats that line up with the demand shift by shift, each site at its students-at-once, never beyond that shift's own need (6,984 for that scenario; the 72 between it and placed are three class days). The scheduler now opens on those base levers; the calendar's roster was placed with the looser roster levers, and the line under the levers says which levers differ.

**The raw rooms bar retired.** Asked what "seats at the sites that count" meant, the honest answer was: a room inventory summed over the window, blind to whether the seats fall where the students are and to each site's students-at-once — the figure that had produced both contradictions. It is gone from the panel. The three bars that remain share the day-by-day basis: seats that line up with the demand at the sites that count; of which a preceptor on the site's roster could cover (each site and shift further capped by preceptors × students per preceptor — the lever, else the rooms' own ratio); and if every site counted (every live site, each still at its students-at-once). One sweep over the assets feeds all three, and `supplySeatsLinedUpStaffable` ≤ `supplySeatsLinedUp` ≤ `supplySeatsLinedUpEverySite` ≤ demand by construction; audit B9b′ checks the order. The raw fields (`supplySeatsOnDemandDays`, `supplySeatsPhysicalOnDemandDays`, `supplySeatsStaffableOnDemandDays`) remain on `CapacityHeadroom` for the per-setting table and audit B8/B9. Sandhills under the base levers, four Radiography offerings: demand 11,640 · lined up 6,984 (0.60×) · of which a preceptor could cover 4,365 (0.38×) · if every site counted 11,640 (1.00×) · placed 6,912 (59.4%).

### Surgical Technology's plan (2026-09-22)

The owner's numbers for Sandhills Surgical Technology are in the seed (`prisma/seed.ts`, `prisma/seed-roster.ts`), so every deploy carries them. The North-Star ladder steps up: 6 productive workers for each of 2026 and 2027, 14 a year from 2028 on. One class a year, each on the college's Fall first day: the Class of 2026 (graduated; started Aug 19 2024), 2027 (in program; Aug 18 2025), 2028 (Aug 17 2026), 2029 (Aug 16 2027) and 2030 (Aug 21 2028). The two small classes carry their own rates over the family's — a 75% completion rate with everyone after that retained — so the chain reads exactly as stated, 8 enrolled → 6 completing → 6 licensed → 6 placed → 6 productive, and each was locked in at 8 seats; the three from 2028 on inherit the family's rates and 19 seats. Their rosters are pinned rather than generated (`seedOfferingStudents` takes `pins`): the Class of 2026 has 8 students, 6 completed and 2 withdrawn, with a completed course record for every term; the Class of 2027 has 8 enrolled, 2 of them withdrawn. Stage actuals come from those records as everywhere else, so the goal page reads 8 enrolled → 6 completing for 2026 with nothing past that (no licensure, placement or productivity record exists). The graduated class is the one completed cohort with students; it carries grades but no clinical shifts and no requirement entries — the scheduler never placed it, and a "scheduled" shift with no seat would be a false record.

### Graduated classes are records (2026-09-22)

The owner could not find the Class of 2026 — Radiography or Surgical Technology — in the analytics. The cause was architectural: the seed rostered planned and running offerings only (Radiography's Class of 2026 had no students; Surgical Technology's, pinned the day before, had grades and nothing past "completing"), and every capacity surface dropped completed cohorts by status (`status in [planned, active]` in `capacityModelFor` and fifteen other queries and actions), so a graduated class vanished from coverage, staffing, site capacity, the scheduler, the map, room utilization, the expansion baseline and the exception queue, and its own page hid its staffing and calendar sections. The analytics pickers were built from students, so an empty class was not even an option.

**One rule** (`src/lib/cohortscope.ts`): every offering that is not archived is a record — planned, running or graduated. A forward-looking surface narrows by date, never by status: `cohortsOverlapping` picks the classes whose terms touch a window, `plannedWindow` opens the scheduler and the site-capacity match on the current academic year (history back to the first graduated class stays selectable), the expansion baseline keeps rows from today on, room utilization keeps classes with a term still to end, and `cohortStatusOn` derives planned / active / completed from dates wherever a status is set. A lint test (`test/cohortscope-lint.test.ts`) fails the suite if a planned+active or enrolled+admitted filter comes back. Rosters that read a class read everyone who sat in it (`ENROLLED_AND_BEYOND`, `ROSTER_STATUSES`).

**The seed writes history through the same steps as a running class.** A graduated class's learners end where the class's own pipeline rates say (`src/lib/cohorthistory.ts` `outcomeMix` / `assignOutcomes`: completers = seats × completion, then licensure, placement and productivity, whole people, the chain never rising; a pinned withdrawal count overrides the first step), with a completion date on the class's last day. Its clinical shifts exist, the scheduler seats the whole past (the roster window now opens at the earliest graduated start — 2024-08-19 at Sandhills), and `seedLearnerRecords` — now run for every college — logs them completed / absent / excused with hours, preceptor and setting, writes a letter grade and points for every finished term (the demo grade bank) and a GPA per learner, and never flips a graduated class back to running. Requirement entries cover completers too; a rotation's placement reads completed / active / planned by its dates. A dummy name is never reused across offerings, so nothing that keys by name can merge two people.

Sandhills after the reseed: Radiography Class of 2026 — 21 students, 3 withdrawn, 2 completed, 1 licensed, 15 fully productive; stage actuals 47 → 31 → 26 → 21 → 18 → 16 → 15 → 15 (its goal); 2,511 completed clinical shifts, 2,334 on a booked seat, GPA on every graduate. Surgical Technology Class of 2026 — 8 students, 2 withdrawn, 6 fully productive; 18 → 12 → 10 → 8 → 6 → 6 → 6 → 6. All 36 completed cohorts across the four colleges carry a roster; no graduate has a shift still "scheduled".

**The analytics read it.** Learner analytics builds its institution / program / cohort pickers from the offerings (a class with no students reads "(no students)", a graduated one "· graduated"), adds a Class year dimension and filter and a time-to-complete tile (median months from first day to completion date). The student directory filters by class. The offering page's Students tile shows the completion rate once the class has ended. The goal page carries "Outcomes by class" — produced against goal and demand per class year, and each class's timeline (the `FamilyAnalytics` component, previously unmounted; `PipelineAnalytics` was dead and is deleted). Home shows the latest graduated class against the goal its year carried. Site load labels a student with their program and class, so two people who share a name are never one filter (the 2,295-hour "student" in the owner's CSV was two learners named Theo Clark in two classes; each carries 1,147 clinical hours, the template's 1,152 less holidays).

**The audit proves it** (`scripts/audit-accuracy.ts` section H, over every class past its last day): no graduate's shift still scheduled, every learner terminal with a completion date and a GPA, all eight stage actuals present and never rising along the ladder, every completed clinical shift on a booked seat (an error where the college has sites, a warning where it has none), every rotation placement completed. Section B now runs over the full range, history included.

### Supervision on a shift: instructors carried from the plan to every reader (2026-09-22)

The owner asked where the instructors were in the clinical site load export and the analytics; there were none. The cause was architectural, not cosmetic. The scheduler picks a college instructor for every instructor-led section (`facultyNeeded ≥ 1`) and the plan writer stored that pick as a session-staff row only; the student shift carried a preceptor pin and nothing for the instructor, and the site-load query never read the staff table. So every site-load row, dimension, measure, site stat and CSV column was preceptor-only, instructor-led sites tripped the "no preceptor named" watch, and the rotation export, the exception queue and the offering ledger repeated the gap. Two more facts compounded it: the three Nurse Aide / Medical Assisting colleges had no instructor people in the seed at all, so the scheduler could never name one there; and the supervision model read "Precepted Experience" (252 Sandhills and Roanoke-Chowan sessions) as *unknown, needs review* because the word list matched "preceptor" but not "precepted".

**One record with two roles** (`prisma/schema.prisma` `StudentShift.instructorId`, additive). The plan writer (`src/lib/planwrite.ts`) pins the section's instructor on every student shift beside the preceptor, keys its updates on both, nulls both on unpin and clear, and gives the calendar pattern a lead staff of the preceptor *or* the instructor (never a null overwrite). The seed (`prisma/seed-roster.ts`) gives a college whose clinical sessions need a whole instructor and has none a bench of four per program family (`seedInstructors`, titled so the staffing filters match — the Sandhills dummy faculty already existed), then in the learner-record step fills every dated instructor-led session × section the plan did not staff with the least-loaded instructor free at that date and start time (written as `seed:history`, so a re-plan cannot delete it) and pins the instructor on logged and future shifts. The site-load query (`getSiteLoad`) reads the pin, falls back to the section's pattern staff **by that person's role** (a preceptor never fills the instructor column or the reverse), reads the session's supervision model, and computes the time.

**The time each supervisor spends with the learner is stated on every row** (`src/lib/supervision.ts` `supervisionTime` / `supervisionOnShift`, shared by site load, the rotation export and the offering ledger so the figures agree page to page): the role's hours on the shift (the whole shift × ⌈facultyNeeded⌉ for a whole instructor; a fractional figure — Sandhills' 0.04 on a precepted rotation — is that fraction of the shift, labelled oversight, nobody named; preceptorsNeeded × shift length for preceptors, at least the shift when one is named), the learners on that shift (same cohort × session × section, attended or due — a withdrawn learner's unattended shift and a logged absence are not on it), and this learner's share (hours ÷ learners; 0 when the learner was absent, and 0 when nobody is named in a required role — the template's hours on the shift are still stated, so a "nobody named" row reads what is owed and that no one gave it). Summed over a site, a person or a class the shares add back up to the time a named supervisor gave: an instructor with ten learners for six hours gave six hours, 0.6 to each — never sixty.

**Every reader carries it.** Site load: dimensions Instructor, Supervision model, Supervised by (instructor named · preceptor named · instructor and preceptor · nobody named · none required) and Learners per supervisor; measures Instructors, Instructor hours and Preceptor hours (shares summed); CSV columns Instructor, Supervision, Supervised by, Learners on shift, and each role's hours on the shift and this student's share; the leaderboard's Supervision column and detail panel; a Watch tile that counts shifts whose *required* supervisor is missing, either role — a role the template does not require is never a gap. The rotation export's log gains the same columns. The exception queue's unprecepted check became `unsupervisedShifts` (kind key kept), with the preceptor / instructor split in the title and the fix. The offering ledger counts a missing preceptor where needed or a missing instructor where needed and states the supervisor hours each learner received. The staffing day bars count a whole instructor per instructor-led clinical section (fractional oversight stays hours, as the scheduler does). The organization record's "Booked here" column reads Lead staff with the role.

**The instructor of record on a precepted rotation (corrected the same day).** The first cut named nobody on a shift whose faculty figure was a fraction (Sandhills' 0.04–0.05 of an instructor on a precepted rotation): the row read "oversight only" with 0.29–0.34 h and no name, so the site-load export at Sandhills showed no faculty member on any shift and the page hid the Instructor lookup (one value only). That was wrong: the fraction is the time the college instructor of record gives each precepted learner, and that instructor is a person the college names. Now the scheduler names the least-loaded instructor on every shift the template gives faculty time to — a whole person is tied up for the shift (facultyNeeded ≥ 1), an oversight instructor is not (one oversees many precepted learners at once) — carrying the instructor's hours (shift × ⌈facultyNeeded⌉, or shift × the fraction) into the plan's staff row; the seed's fill pass does the same for history; site load, the CSV, the rotation export and the ledger name the instructor with that time per student; a shift with faculty time and nobody named is a gap (the supervision reading requires the role whenever facultyNeeded > 0), and audit F1 checks every such completed shift.

**Bench size and double-booking.** A flat bench of four left four Lenoir sections unstaffed at a weekday and start time where five sections met at once, so `seedInstructors` sizes each family's bench to the peak of clinical sections meeting on the same weekday and start time in overlapping terms (never fewer than four), and the fill pass first marks every plan-assigned instructor busy at their date × start time across all offerings before filling, so no fill can double-book one. A slot where every instructor is on a section stays unstaffed and the audit says so (F1 ahead).

**Numbers after the reseed** are in the commit's verification (audit section F: F1 every completed instructor-led shift names an instructor, F2 the pin is one of the section's staff rows, F3 site load names an instructor on at least every pinned shift; F1-ahead warns about upcoming shifts the scheduler could not place — those have no site and no instructor, and the exception queue shows them). Assumption stated: the Nurse Aide colleges' instructors are dummies, as Sandhills' faculty already were; a college replaces them with its own people and the same pins follow.

### Surgical Technology (revised): a second template with the program's meeting pattern (2026-09-22)

The owner asked for a second copy of Sandhills' Surgical Technology program carrying the meeting pattern the program stated, visible on Design & sequence. Two things had to exist first. A full copy (`src/lib/programcopy.ts` `copyProgram`): the Duplicate button's copy dropped the family, the launch cadence, every session's start and end time and section times, the contact policies, homework and the course-level clinical requirements, and it copied offerings (records of classes that ran) into a template — it now copies everything the template holds and no offerings, and the seed uses the same function. And the pattern itself as a pure, tested transform (`src/lib/surgtechrevision.ts`, `test/surgtechrevision.test.ts`), applied by `prisma/seed-surgtech-revised.ts` to the copy "Surgical Technology (revised)" on every seed.

What changed, course by course — and why SUR 111's Thursday and SUR 123 could not be seen before. The workbook carried SUR 111's Thursday 8:30–3:00 lab block tagged "Online / Internet"; every calendar and section reads that as no meeting, so the block never appeared anywhere. It is now two in-person lab blocks each week, 8:30–11:30 in the classroom (Kennedy Hall 148) and 12:00–3:00 in the lab (Kennedy Hall Lab 129), keeping the catalog's 6 lab hours a week beside the Monday class. SUR 122 meets every Thursday 8:30–11:00 (class) and 12:00–3:00 (lab, in person, the full block — the workbook's duplicate online lab rows are gone; the catalog's other 2.5 class hours stay online). SUR 123's clinicals are Monday and Tuesday 6:30–3:00 and Friday 6:30–12:30 (the workbook had 8:30 starts), 23 clinical hours a week, still 1:1 with a preceptor — per-site capacity ("some sites take more than one per rotation") is the site's students-at-once on its record, never the template's; every Surgical Technology site's is currently unknown, and the exception queue says so. Term 3 the same: SUR 134 Thursday 8:30–11:00 and 12:00–3:00 in the classroom (its online share is the remaining 2.5 of 8 class hours), SUR 135 Mon/Tue 6:30–3:00 and Fri 6:30–12:30. SUR 210 runs every Wednesday and Thursday 6:30–3:00 in Term 4 (the workbook had it in Term 5 on alternating weeks; the program places it in Term 4, and the copy does — 32 shifts, 17 clinical hours a week; Term 5 keeps SUR 137). The program spoke of a Tuesday meeting for SUR 111; the workbook has none and none is invented. The original program keeps every class that ran or is planned; the copy has no offerings until one is created for it.

### The deploy hardened (2026-09-23)

The hosted site showed Next.js's server-exception overlay on every page after the instructor-of-record deploy. The whole deploy stack was reproduced in the sandbox — the derived Postgres schema, `prisma db push --force-reset`, the seed and the production build against a real PostgreSQL 16, then every route through a logged-in browser, and the same against an empty database — and every page rendered, so the failure was Vercel-runtime-specific and the setup had no defence against the three ways that runtime fails: the build reset the live database *before* building (a code error or a seed past Vercel's 45-minute limit leaves the previous deployment live against a wiped database); the client used Prisma's 5-second connect timeout against a serverless Postgres that scales to zero and takes longer to wake; and the seed made ~35,000 single-row round trips (fine on a local socket, minutes over a network). Now `vercel-build` builds first and touches the database last; `src/lib/db.ts` builds the connection string with a 30-second connect and pool timeout and fails with a message naming the variable when none is set; the seed groups its writes — one `updateMany` per distinct logged value set, pin groups, counters in transactions of a hundred, requirement logs, shift assignments and cohort dates through `createMany` in chunks — and prints ⏱ laps per phase in the build log. `docs/DEPLOY.md` says what a failed seed leaves behind and where to look.

### The deployment carries its own database (2026-09-23)

The Neon store behind the Vercel project died on Neon's side (its data browser could not load it; every build was refused at the connection within a tenth of a second) while the last good deployment was still serving, and the owner could not use the site. Since every deploy already rebuilt the whole database from the seed, the database was disposable — so it now lives inside the deployment: `scripts/vercel-build.sh` seeds `prisma/rosie.db` (SQLite, ~60 MB) at build time and `next.config.mjs` traces it and Prisma's engines into every function; `src/lib/db.ts` copies it to `/tmp` on first use and opens it there. There is no service to expire, and a failed build leaves the previous deployment serving its own bundled data. Edits on the hosted site persist within an instance only; a hosted Postgres stays an opt-in (`ROSIE_DB=postgres`) for persistent editing, with the hardened build-first order and the 30-second cold-start allowance from the earlier fix. `test/db.test.ts` pins the resolution: bundled file → `/tmp` copy once; Postgres opt-in with timeouts; local `DATABASE_URL`; a bare production process names what is missing.

**The first bundled deploy shipped a file without its placements (fixed the same hour).** Vercel sets `VERCEL=1` during the build as well as at runtime, so while the seed ran, every step that reaches the database through the app's client (`src/lib/db.ts`: the scheduler placement in `prisma/seed-plan.ts`, the plan writer, the stage actuals, the site registry) took the hosted branch, copied the half-seeded file to `/tmp` and wrote there; the bundled file shipped without those rows and the site load page read empty. `scripts/vercel-build.sh` now names the bundled file in `ROSIE_DB_FILE` for the push and the seed, which `datasourceUrl` honours before anything else, so the seed's client and the app's client write one file; `next build` runs with it unset. `test/db.test.ts` pins it; the verification compares row counts in the bundled file against `/tmp` after a build run with `VERCEL=1`.

### Surgical Technology: the owner's sheet is the program's source (2026-09-23)

The owner supplied the corrected "Raw Data & Calculations" table for Sandhills Surgical Technology and named it the source of truth. It is saved row for row as `prisma/templates/source/surgtech-raw-data.tsv`; `scripts/import-surgtech-pack.mjs` rebuilds `prisma/templates/surgtech.json` from it (columns by header text, nothing reinterpreted; the only derived values are each row's start and end time read from its notes for the row's own weekday, the back-to-back term windows from each term's highest session week, and each course's weekly hours by kind), and `createPackProgram` builds the program from the pack as before. The interim "Surgical Technology (revised)" copy and its transform are retired: the sheet supersedes them. What the sheet changes: SUR 111 meets Monday 8:30–9:20, Tuesday 8:30–2:30 (5 h class, three faculty) and Thursday 8:30–3:00 (6 h lab, two faculty), all in person — the old workbook had the Thursday block tagged online, which is why it never appeared; SUR 122's lab is Thursday 12–2 (3 h, in person, no online lab rows); SUR 123's clinicals are Monday and Tuesday 6:30–3:00 and Friday 6:30–12:30, weeks 1–10; SUR 135's are Monday and Tuesday 6:30–3:30 and Friday 6:30–12:30 on site, weeks 1–10; SUR 210 alone in Term 4; SUR 211 and SUR 137 in Term 5.

Imported as written, for the owner to correct in the sheet if they are slips (a re-paste and `node scripts/import-surgtech-pack.mjs` is the fix path): SUR 210's weeks 1–6 are six rows of kind Clinical tagged Online / Internet, 1 hour, 20 students, faculty 1.5 or 0, titled like SUR 137's last sessions — they read as online rows with no clinical shift, so SUR 210's clinicals start in week 7 (one 6-hour shift a week, Wednesday in odd weeks with the 3-hour faculty rule, Thursday in even weeks); SUR 123 stops at week 10 of a 16-week term; three SUR 135 shifts (week 3 Monday, week 4 Friday, week 5 Monday) carry 0 preceptors and read "none required" for that role; SUR 135's thirty shifts carry the rotation type "Other" (the workbook said "Other (surgical rotations)", which the taxonomy could read as an operating-room rotation), a wording that maps to no setting — so those shifts are demand with no setting (A3 reports them, the scheduler leaves them unplaced as setting-rule-unmet, and no site-load row exists for them) until the sheet names the rotation or the rotation settings page maps "Other" for Sandhills.

Two rules the sheet's slip exposed, fixed across the engine rather than around the rows:

- **An online session is never a clinical shift.** The calendar, the learner records and the ledger already read an online session (delivery mode online, or location Internet) as nothing to attend on a date, but clinical demand did not: the scheduler booked SUR 210's six online "Clinical" rows as 20-seat shifts, so the audit read seven students on one-seat operating-room suites (C3, D2). One predicate, `isOnlineSession` in `lib/capacitymodel`, now serves every reader (demand, the master calendar, the calendarizer, the seed's learner records, the instructor bench and the instructor-of-record fill); `clinicalDemandRows` drops online clinical rows (and `clinicalDemand`, the older dated view, with it), and the seed writes no student shift for one, so no demand, seat, booking, pin, shift or site-load row ever comes from an online session, whatever weekday and time the sheet gives it. The audit's E3 now compares the asset map's cells to the demand that has a setting (unmapped demand is A3's report, and reaches no cell by design).
- **A student is pinned to the room that seats them.** When one section spreads across several rooms at a site (the plan's `parts`), the bookings were written per room but every student of the section was pinned to the lead room, so the roster read more students on that room than it has seats. `roomForSeat` in `lib/planwrite` deals the section's students to the rooms in seat order (the same order the bookings' seats follow), so a pin, its booking and the site load name the same room. Precepted rotations with more than one student per section (the owner's note that some sites take more than one per rotation) are the case that reaches it.

### Precise and realistic offering starts on the goal page; Carteret's 2025–26 calendar (2026-09-24)

**What was wrong.** A goal-page slot's start list (`lib/goaldates candidateStarts`) kept only starts on or after today, so a year already under way (Carteret's 2026 Nurse Aide classes, entered to compare with the plan) offered a handful of late-autumn Mondays; the "type any date" box beside it was small and unlabelled. For semester programs a calendar year with any coded semester start dropped the pattern starts of that year's other seasons (Radiography 2030 had no start), and pattern starts were the raw anchor day rather than the Monday on or after it. The college's own part-of-term starts (16/14/12/10/8-week sessions) were never suggested, and their ends were never stored: the calendar's range lines ("First 8 week classes | Aug 13 – Oct 8") were coded `other`. A lock-in stored `status: "planned"` for good; nothing in the app moved it with the dates.

**What changed.**
- A start is never cut at today. Every start whose aligned end lands in the goal year is offered, and says "already started" / "already finished" (`timing`, from `lib/cohortscope cohortStatusOn`). The suggested starts (a new allocation's slots) prefer the starts still ahead, else the year's own.
- The college's own starts come first. The parser keeps a dated range line whose label only names a session or the semester as a start **with its end** (`classifyRange`); a session's length is the count its label names ("Second 8 week classes" → 8, whatever its dates count), else its dates rounded to weeks when an end is coded, else null (missing ≠ zero; `weeksNamed`, `sessionWeeksOf`). A date-only "begins" on a day that already has a start with its end adds nothing and is dropped; two starts with the same first and last day are one event; starts on one day with different ends (16-week, first 10-week, first 8-week) stay distinct; within a season-year the earliest start is the semester's, on a tie the one named as the full term, else the longest. A range longer than 26 weeks is a typo, coded `other` with a warning (Carteret's "Winter term classes | Nov 18, 2025 – Dec 29, 2026").
- A continuing-education class is offered the coded starts (semester and sessions) and any open Monday; suggested first the sessions whose length equals the program's first term (an 8-week class starts with an 8-week session), then any session long enough, then any coded start, then any Monday. Of several starts on one day the one named is the session that fits, else the shortest long enough, else the longest. A class that starts with a coded session of its own length ends with that session (`endSource "calendar"`, `lib/termalign`); a shorter class ends after its own weeks.
- Semester programs: coverage per season **and** year, as `lib/term nextSemesterStart` reads it — a coded season-year uses its date, every other one the Monday on or after the anchor; term 1's season, else the program's launch seasons, filters.
- The list is grouped (College calendar starts · Any Monday · Semester pattern) and the date box is a first-class "or any date" control; a typed date that a semester program snaps to its coded start says where it lands.
- An offering's status follows its dates wherever its dates are written (`alignOfferingToCalendar`): a start behind us is active, a last day behind us is completed — a recorded offering never reads "planned". Changing a program's calendar mode re-aligns every dated offering, not only planned ones. Audit H0 lifecycle warns on any stored status the dates contradict.
- A calendar paste replaces only its own span (first to last coded day), so Carteret's 2025–26 and 2026–27 calendars coexist; the seed file carries both years (`prisma/seed-data/calendars/carteret.txt`): term starts 2025-08-13, 2026-01-07, 2026-06-01, 2026-08-17, 2027-01-06, 2027-06-01, every session with its end.
...
### Radiography clinicals: a tenth of a faculty member on every clinical shift; every lab in Kennedy Hall 147; RAD 112's lab moved off its class (2026-09-24)
...
**What changed.** All 146 clinical sessions of those five courses read `facultyNeeded 0.1` (the owner's rule; a first pass at 0.25 was corrected the same day): an 8-hour shift gives each 1:1 student 0.8 h of instructor time, RAD 171's 9-hour shift 0.9 h, RAD 151's 7-hour 0.7 h — the instructor of record is still fractional oversight (not tied up by the shift), so the scheduler, the site load and the staffing need read it as before at the new figure. The seed's fallback clinical profile for a generated program follows the same rule. RAD-231's sixteen lab rows read Kennedy Hall 104 while every other lab is in Kennedy Hall 147; all 120 labs are now in Kennedy Hall 147 (the RAD-231 and RAD-271 class rows in Kennedy Hall 104 are unchanged). RAD 112's lab section 2 is Wednesday 2:00p–4:50p (the same 2 h 50 m block as section 1, ten minutes after the class ends); the class and section 1 are unchanged. `test/packs.test.ts` pins all three.
