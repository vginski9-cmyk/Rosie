...
**Surgical Technology's class size (2026-09-21).** The owner's figure for the program's maximum cohort is 23, not the workbook's modeled 19: `prisma/templates/surgtech.json` `maxCohort` is 23 (`modeledEnrollment` stays the workbook's 19), so `defaultCohortSeats` — the cap the goal planner splits a goal by — is 23. The workbook's class and lab sessions still hold 20 each, so a full class of 23 runs as two sections of those sessions; that is the sheet's figure, not a change made here.
