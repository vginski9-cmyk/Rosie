...
// (2026-09-24): a quarter of a faculty member on every clinical session, and RAD 112's class and lab
// sections never overlapping.
interface PackSession { kind: string; number: number; dayOfWeek: string | null; startTime: string | null; endTime: string | null; facultyNeeded: number; sectionTimes: string | null }
...
describe("the Radiography pack", () => {
  it("gives every clinical session of RAD 151, 161, 171, 251 and 261 a quarter of a faculty member", () => {
    ...
    expect(clinical.every((s) => s.facultyNeeded === 0.25)).toBe(true);
